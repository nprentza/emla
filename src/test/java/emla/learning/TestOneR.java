...
public class TestOneR {
	...
	public void testLoadDataset() {
		...
		List<FrequencyTable> frequencyTables = emlaSession.calculateFrequencyTables(ds, "train");
		...
		Dataset ds_R = emlaSession.copyData(ds, caseIDs);
		List<FrequencyTable> frequencyTables_R = emlaSession.calculateFrequencyTables(ds_R, "train");
		
		frequencyTables_R.forEach(f_R -> System.out.println(f_R.toString()));
		
		System.out.println("\n\n ** BEST FREQUENCY ** ");
		Frequency f_R = emlaSession.calculateFrequencyHighCoverageLowError(ds_R, "train");
